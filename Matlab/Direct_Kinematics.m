...
% Komodo Arm kinematics description, followed by Plots and "Inverse"
f = figure;
f.Renderer = 'painters';
...
z1x = 0.*t1x;                     
z1y = h.*ones(1,length(t1x)) + H;
t1y = x.*sind(q) + l2.*sind(q-f) + H; 
color = [0.7 0.7 0.7];
...
for i= 1:8:length(t1x)
    ...
    xplot=[p1(i),(p1(i)+Xp(j))/2-10, Xp(j)]; yplot = [p3(i),(p3(i)+Yp(j))/2-20, Yp(j)];
...
yp =bsxfun(@plus, x(col).*sind(q(col)) + l33.*sind(q(col) - b(col)) + H, lp.*sind(delta(ind))');
