...
hold on                                                      %Plot Path
vx=diff(centerx(2:end)).*rate / 720;                         %Compute Vx by Vx=(X(i)-X(i-1))/dt , first order dervative                 
vy=diff(centery(2:end)).*rate / 576;                         %Compute Vy by Vy=(Y(i)-Y(i-1))/dt , first order dervative 
V= (vx.^2+vy.^2).^(1/2);        
plot(centerx(2:end),centery(2:end),'r')
figure(2)                                                    %Display velocity over time

plot((1/rate)*(1:length(V)),V,'o');grid minor                %Smooths the data in the column vector y using a moving average filter
hold on
